...
'''
Learning Neural Networks (MLPregressor)!
'''
...
'''
Learning Neural Networks (MLPclassifier)!
'''
# import random as rand
# import numpy as np
# from sklearn import exceptions
...
# clf_warm = MLPClassifier(solver='sgd', alpha=0.1, hidden_layer_sizes=(10, 5), max_iter=2, warm_start=True)
# # clf_cold = MLPRegressor(solver='lbfgs', alpha=0.1, hidden_layer_sizes=(10,), max_iter=1, warm_start=False)

# def f(x):
#     value = list(i ** 2 for i in x)
#     c = list(0 if np.sum(v) < 100 else (1 if np.sum(v) < 200 else 2) for v in value)
#     return np.array(c)
