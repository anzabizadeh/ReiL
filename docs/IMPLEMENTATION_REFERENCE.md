...
**`PPOTandemModel`** ([ppo_learner_tandem.py:46](../reil/learners/ppo_learner_tandem.py#L46)) — chained multi-head ("tandem") actor. Rebuilt in the **2026-07-06 Paper-3 restart** (Ch. 3 is a guiding document only; clean break, no back-compat with the retired `backprop_mode`/`training_switch` vocabulary). Key differences vs. `PPOModel`:
- `actor_layer_sizes: dict[str, tuple[int, ...]]` — one entry per head, in head order, plus an **optional `'trunk'` entry**: the trunk MLP produces a shared latent `z` that replaces the raw input as every section's input and is trained by every head (justification: a representation learned from all heads). Without `'trunk'`, sections read the raw input. Section j>0 reads `concat(z, coupling)`, the coupling being the previous section's logits, `LayerNorm`-normalised (`'batch'` is avoided: the actor is always called `training=False`, so BatchNorm stats never update). Every section always trains on its own head's loss.
- `coupling_gradient: Literal['full','blocked','gated'] = 'full'` — routes the coupling gradient only. `'full'` = downstream losses also train upstream sections (the *only* behaviour any pre-restart run had, whatever it was called); `'blocked'` = `stop_gradient` on the coupling; `'gated'` = `GradientGate` on the coupling, opened by the schedule (== `'blocked'` without one).
- `head_loss_weights: tuple[float, ...] | None` — per-head multiplier on the PPO+entropy loss (None = all 1.0). `(0.0, 1.0)` + `coupling_gradient='full'` = **downstream-only** training, i.e. the dissertation §3.2.2 `shared` mode that never existed before (resolves Q-CH3-BACKPROP-NAMING).
- `training_schedule: dict[str, int] | None` — alternating-freeze schedule, `{phase: train-step count}` cycled in key order. Phases: `'all'`, `'heads'` (the `_output` Dense layers), `'trunk'` (the `_trunk_` layers if a trunk exists, else all non-head layers; also opens `'gated'` couplings), a section name, or any layer-name substring. Heads now carry their section (`actor_dose_output`), so sections are addressable by name. Implemented via per-variable gradient-mask `tf.Variable`s read inside the single `train_actor` trace — `layer.trainable` toggling never reached the traced graph, so the retired switch silently did nothing.
- `_advance_training_phase()` advances the schedule; `_apply_training_phase()` assigns masks + gates; both rebuilt on unpickle. The group-lasso regularizer (`_compute_regularizer_loss`) forges the **dose head only** (action forging is dose-table explainability; duration actions are never eliminated), summed once per iteration. `per_neuron_l2_vector` still spans all heads for logging, but **only its dose slice `[:action_per_head[0]]` is meaningful** — the duration tail is never regularized, so it is a ~constant that adds a fixed offset (`action_per_head[1]`) to the `n_actions_alive` diagnostic. Subtract it in any dose-action-count analysis.
