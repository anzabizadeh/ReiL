...
subjects={'Warfarin_v4_default_state': WarfarinModel_v4(max_day=max_day,
...
exp.generate_subjects(number_of_subjects=1000)

for agent in agents:
        exp.assign([(agent, 'Warfarin_v4_default_state')])
        print(agent)
        exp.run()
        exp.divest([(agent, 'Warfarin_v4_default_state')])
